...
export function AIChat() {
    ...
    try {
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          context: 'medical' // For medical-specific responses
        }),
      });
  ...
  return (
    <div 
      className="fixed bottom-8 right-8 rounded-2xl flex flex-col overflow-hidden" 
      style={{ 
        width: 'min(400px, 90vw)', // Responsive: max 400px but scales down on small screens
        height: 'min(540px, 80vh)', // Responsive: max 540px but scales down on small screens
        minWidth: 'min(320px, 85vw)', // Minimum size but still responsive
        minHeight: 'min(400px, 70vh)', // Minimum size but still responsive
      ...
      >
        ...
        >
          <div 
            ...
            style={{
              width: isListening ? 'min(280px, 70vw)' : 'min(200px, 50vw)',
              height: isListening ? 'min(280px, 70vw)' : 'min(200px, 50vw)',
              minWidth: isListening ? 'min(200px, 50vw)' : 'min(140px, 35vw)',
              minHeight: isListening ? 'min(200px, 50vw)' : 'min(140px, 35vw)',
              maxWidth: '80vw',
              maxHeight: '60vh'
            }}
          >
            ...
            <div
              ...
              style={{
                ...
                boxShadow: message.sender === 'user'
                  ? '0 0 15px rgba(73, 100, 254, 0.2), inset 0 0 10px rgba(201, 100, 255, 0.1)'
                  : '0 0 15px rgba(138, 43, 226, 0.2), inset 0 0 10px rgba(138, 43, 226, 0.1)'
              }}
            >
      ...
      >
        ...
        >
          <div 
            className="transition-all duration-500"
            style={{
              width: isListening ? '340px' : '280px',
              height: isListening ? '340px' : '280px',
              minWidth: isListening ? '340px' : '280px',
              minHeight: isListening ? '340px' : '280px'
            }}
          >
            <Orb
              hoverIntensity={isListening ? 0.6 : (isTyping ? 0.4 : 0.2)}
